...
public class AttendanceDAOImpl implements AttendanceDAO {
    ...
    @Override
    public boolean updateAttendanceStatus(int studentId, int courseId, LocalDate date, String newStatus) {
        String query = "UPDATE attendance SET status = ? WHERE student_id = ? AND course_id = ? AND attendance_date = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(query)) {
    ...
    @Override
    public boolean deleteAttendance(int studentId, int courseId, LocalDate date) {
        String query = "DELETE FROM attendance WHERE student_id = ? AND course_id = ? AND attendance_date = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(query)) {
            ps.setInt(1, studentId);
            ps.setInt(2, courseId);
